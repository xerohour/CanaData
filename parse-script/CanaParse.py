...
class CanaParse:
    ...
    def apply_filters(self):
        ...
        # Pre-compute string representation for each row once
        # This prevents O(num_filters * num_rows) repeated join/lower operations
        row_strs = [" ".join([str(x) for x in row]).lower()
        ...
        for f in self.filters:
            ...
            # Apply filters using pre-computed string representations
            filtered: List[Any] = [
                # copy row to avoid mutating raw_data
                row[:] for row, row_str in zip(self.raw_data, row_strs)
