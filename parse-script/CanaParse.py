...
class CanaParse:
    ...
    def apply_filters(self):
        ...
        # Pre-calculate row_str exactly once per row to avoid redundant string joins across multiple filters
        self.raw_data_with_str = [(row, " ".join([str(x) for x in row]).lower()) for row in self.raw_data]
        ...
        for f in self.filters:
            ...
            # Apply filters
            filtered: List[Any] = []
            for row_tuple in self.raw_data_with_str:
                row, row_str = row_tuple
                # Evaluate condition on original row
                if self.is_match(row, f, price_col, row_str):
                    filtered.append(row[:]) # Only copy row if it's a match
