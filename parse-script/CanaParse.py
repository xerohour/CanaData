...
class CanaParse:
        ...
        for f in self.filters:
            ...
            if f.limit_results_amt > -1 and len(filtered) > f.limit_results_amt:
                filtered = sorted(
                    filtered,
                    key=lambda x: (
                        float(str(x[price_col]))
                        if str(x[price_col]).replace(".", "", 1).isdigit()
                        else 999999
                    ),
                )
    ...
    def is_match(self, row, f, price_col):
        ...
        if f.price:
            ...
            row_price = (
                float(row_price_raw)
                if row_price_raw.replace(".", "", 1).isdigit()
                else 0
            )
        ...
        with tag("div", id=section_id):
            ...
            if not results:
                with tag(
                    "div",
                    klass="empty-state",
                    style="text-align: center; padding: 3rem 1rem; background: var(--glass); border-radius: 12px; border: 1px dashed var(--glass-border); margin: 1rem 0;",
                ):
                    with tag(
                        "div",
                        ("aria-hidden", "true"),
                        ...
                        text("🔍")
                    with tag("h4", style="color: var(--text); margin-bottom: 0.5rem;"):
                        text("No results found")
                    with tag("p", style="color: var(--text-muted); font-size: 0.9rem;"):
                        text("Try adjusting your filter criteria to see more products.")
                return
        ...
        with tag("tr"):
            ...
            with tag("td", klass="thumb"):
                ...
                if img_url:
                    with tag(
                        "a",
                        ("aria-label", f"View larger image of {product_name}"),
            ...
            if f.cbd_floor > 0:
                cbd_val = next(
                    (str(s).split("+")[1] for s in row if str(s).startswith("cbd+")),
                    "0",
                )
                with tag("td"):
