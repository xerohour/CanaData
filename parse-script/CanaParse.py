...
class CanaParse:
    ...
    def _generate_navbar(self, doc, tag, text):
        ...
        with tag('nav', klass="navbar"):
            ...
            with tag('div'), tag('ul', klass="navbar-nav"):
                for f in self.filters:
                    with tag('li'), tag('a', klass="nav-link", href=f'#{f.name.replace(" ", "_").lower()}'):
                        text(f.name)
    ...
    def _generate_row(self, doc, tag, text, row, f):
        ...
        with tag('tr'):
            ...
            # Category
            with tag('td'), tag('span', style="background: rgba(0, 212, 255, 0.1); color: var(--secondary); padding: 2px 8px; border-radius: 4px; font-size: 0.8rem;"):
                text(category_name)
