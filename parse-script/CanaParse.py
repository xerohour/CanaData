...
class CanaParse:
    ...
    def apply_filters(self):
        ...
        # Pre-calculate row_str for performance
        raw_data_with_strs = [(row, " ".join([str(x) for x in row]).lower()) for row in self.raw_data]
        ...
        for f in self.filters:
            ...
            # Apply filters
            filtered: List[Any] = []
            for row, row_str in raw_data_with_strs:
                row_copy = row[:] # copy row to avoid mutating raw_data
                if self.is_match(row_copy, f, price_col, row_str):
    ...
    def is_match(self, row, f, price_col, row_str):
        ...
        # 3. Join row for word-based searches (pre-calculated)
        # row_str is passed as parameter
