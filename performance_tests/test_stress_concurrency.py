...
def test_stress_locking():
    ...
    def worker(i):
        for j in range(100):
            scraper.results_queue.put({'id': i * 100 + j})
            time.sleep(0.001)
    ...
    # Emulate the main thread consuming the queue
    while not scraper.results_queue.empty():
        scraper.allMenuItems.append(scraper.results_queue.get())
