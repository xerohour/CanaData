...
## 7. Rate Limiting Concurrency Fix (July 2026)

**Findings:**
- `ConcurrentMenuProcessor` held the `request_lock` during `time.sleep()` for rate limiting, forcing threads to block one another sequentially and neutralizing parallelization.
- The stress benchmarks were updated to replace artificial `time.sleep()` calls with real invocations to `process_menu_items_json` to genuinely test lock contention.

**Optimizations:**
- Moved the `time.sleep()` operation outside the `request_lock` block while keeping the required logic for scheduling `last_request_time`. This perfectly staggers threads, allowing concurrent sleeping and executing rate limiting efficiently.
