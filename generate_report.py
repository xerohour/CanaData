...
def generate_html_report(data, region_name="Colorado"):
    ...
    *:focus-visible {
    ...
    safe_region_name = html.escape(region_name)
    html_content = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    for item in listings:
        ...
        safe_name = html.escape(str(item.get('name', 'N/A')))
        safe_type = html.escape(str(item.get('type', 'N/A')))
        safe_address = html.escape(str(item.get('address', 'N/A')))
        safe_city = html.escape(str(item.get('city', 'N/A')))
        safe_hours = html.escape(str(item.get('todays_hours_str', 'N/A')))
        safe_phone = html.escape(str(item.get('phone_number', 'N/A')))
        menu_items_count = html.escape(str(item.get('menu_items_count', 0)))
        safe_license = html.escape(str(item.get('license_type', 'Recreational')))
        safe_web_url = _safe_url(item.get('web_url'))
