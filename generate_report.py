...
def generate_html_report(data, region_name="Colorado"):
    ...
    *:focus-visible {
    ...
    safe_region_name = html.escape(str(region_name))
    safe_total_listings = html.escape(str(total_listings))
    html_content = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>Weedmaps Discovery Report - {safe_region_name}</title>
    ...
    <body>
        ...
        <div class="container" id="main-content">
            <header>
                <h1>{safe_region_name} Discovery</h1>
                <p class="meta-summary">Found {safe_total_listings} matches in the region • Generated on {datetime.now().strftime('%b %d, %Y')}</p>
            </header>
    ...
    for item in listings:
        avatar_data = item.get('avatar_image')
        if not isinstance(avatar_data, dict):
            avatar_data = {}
        raw_avatar = str(avatar_data.get('original_url') or 'https://images.weedmaps.com/static/avatar/dispensary.png')
        if not (raw_avatar.startswith('http://') or raw_avatar.startswith('https://')):
            raw_avatar = 'https://images.weedmaps.com/static/avatar/dispensary.png'
        avatar = html.escape(raw_avatar)

        rating = html.escape(str(item.get('rating') or 'N/A'))
        reviews = html.escape(str(item.get('reviews_count') or 0))
        is_open = item.get('open_now') or False
        ...
        promo_html = ""
        if promo:
            safe_promo_code = html.escape(str(promo.get('code') or 'Special Offer'))
            safe_promo_title = html.escape(str(promo.get('title') or 'Check website for details'))
            promo_html = f"""
        ...
        raw_web_url = str(item.get('web_url') or '#')
        if raw_web_url != '#' and not (raw_web_url.startswith('http://') or raw_web_url.startswith('https://')):
            raw_web_url = '#'
        safe_web_url = html.escape(raw_web_url)

        html_content += f"""
                <div class="card">
                    <div class="card-header">
                        <img src="{avatar}" alt="{safe_name}" class="avatar">
                    ...
                    <div class="footer-actions">
                        ...
                        <a href="{safe_web_url}" target="_blank" rel="noopener noreferrer" aria-label="View {safe_name} on Weedmaps" class="btn btn-primary">View on Weedmaps</a>
                    </div>
                </div>
        """
