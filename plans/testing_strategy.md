...
class TestIntegration:
    ...
    def test_complete_weedmaps_workflow(self, canadata_instance):
        ...
        menu_url_1 = (
            "https://weedmaps.com/api/web/v1/listings/dispensary-1/menu?type=dispensary"
        )
        menu_response_1 = {
            "categories": [
                {
        ...
        menu_url_2 = (
            "https://weedmaps.com/api/web/v1/listings/dispensary-2/menu?type=dispensary"
        )
        menu_response_2 = {
            "categories": [
                {
        ...
        assert canadata_instance.finishedMenuItems[0]["name"] in [
            "Blue Dream",
            "OG Kush Hash",
        ]
    ...
    def test_cannmenus_integration(self, canadata_instance, monkeypatch):
        ...
        retailers_response = {
            "data": [
                {
                    "id": "retailer-1",
                    "name": "California Dispensary",
                    "slug": "california-dispensary",
                }
            ]
        }
        ...
        menu_response = {"data": [{"name": "Northern Lights", "price": 45}]}
    ...
    def test_leafly_integration(self, mock_apify_client, canadata_instance):
        ...
        mock_actor_call.wait_for_finish.return_value.get.return_value = {
            "items": [{"name": "Sour Diesel", "price": 55, "type": "flower"}]
        }
...
class TestPerformance:
    ...
    def test_cache_performance_benefit(self, cache_manager):
        ...
        assert (
            cache_time < no_cache_time * 2
        )  # Allow some overhead for cache management
    ...
    def test_data_processing_scalability(self, optimized_processor):
        ...
        for size in sizes:
            ...
            test_data = {
                f"location-{i}": [
                    {"name": f"Product-{j}", "price": j} for j in range(size // 100)
                ]
                for i in range(100)
            }
...
import os
from unittest.mock import Mock, patch
from CanaData import (
    CanaData,
    CacheManager,
    ConcurrentMenuProcessor,
