...
class OptimizedDataProcessor:
    ...
    def _flatten_all_items(self, all_menu_items: Dict[str, List[Dict]]) -> pd.DataFrame:
        ...
        # Optimization: Use fast list comprehension and dict unpacking instead of loops with .copy() and append()
        items_with_location = [
    ...
    def _handle_remaining_nesting(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in nested_columns:
            try:
                ...
                # Optimization: pure Python list comprehension bypasses Pandas Series apply() overhead
                df[col] = [json.dumps(x) if isinstance(x, (dict, list)) else str(x) for x in df[col]]
            except Exception as e:
                logger.warning(f"Failed to flatten column {col}: {e}")
                # Optimization: use native vectorized astype for error recovery
