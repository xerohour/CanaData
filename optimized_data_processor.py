...
class OptimizedDataProcessor:
    ...
    def _handle_remaining_nesting(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            # Bolt optimization: Use first_valid_index instead of dropna to avoid O(N) memory allocation overhead
            if df[col].dtype == 'object':
                first_idx = df[col].first_valid_index()
                if first_idx is not None:
                    val = df[col].loc[first_idx]
                    # Handle case where duplicate indices exist resulting in a Series
        ...
        for col in nested_columns:
            try:
                ...
                # Bolt optimization: Use list comprehension over .apply() lambda which is significantly faster and avoids Pandas Series overhead per row
