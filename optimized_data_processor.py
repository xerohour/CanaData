...
class OptimizedDataProcessor:
    ...
    def _flatten_all_items(self, all_menu_items: Dict[str, List[Dict]]) -> pd.DataFrame:
        ...
        """
        # Collect all items with location info (optimized with dict unpacking and list comp)
        items_with_location = [
    ...
    def _handle_remaining_nesting(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in nested_columns:
            try:
                # Convert to string representation for nested data (optimized with list comprehension)
                # Bypasses pandas Series overhead for element-wise operations (~5-10% speedup)
