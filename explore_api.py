...
def explore_endpoint(name, url):
    ...
    if result and result != 'break':
        ...
        if first_item and isinstance(first_item, dict):
            logger.info(
                "Potential cross-reference fields found in first item:")
            id_fields = [k for k in first_item.keys() if 'id' in k.lower(
            ) or 'slug' in k.lower() or 'license' in k.lower() or 'sku' in k.lower()]
