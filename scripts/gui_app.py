...
class CanaDataGUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(controls, text="Open Output Folder",
    ...
    def _build_run_source_section(self, parent: ttk.Frame) -> None:
        ...
        for text, value in (("Weedmaps", "weedmaps"), ("Leafly", "leafly"), ("CannMenus", "cannmenus")):
            ttk.Radiobutton(
                section, text=text, variable=self.source_var, value=value).pack(anchor=tk.W)
    ...
    def _build_scope_section(self, parent: ttk.Frame) -> None:
        ...
        for text, value in (
            ...
            ttk.Radiobutton(
                section, text=text, variable=self.slug_mode_var, value=value).pack(anchor=tk.W)
    ...
    def _build_data_section(self, parent: ttk.Frame) -> None:
        ...
        section.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 8))
        ttk.Checkbutton(section, text="Storefronts",
                        variable=self.storefronts_var).pack(anchor=tk.W)
        ttk.Checkbutton(section, text="Deliveries",
                        variable=self.deliveries_var).pack(anchor=tk.W)
        ttk.Checkbutton(section, text="Fetch Brands",
                        variable=self.fetch_brands_var).pack(anchor=tk.W)
        ttk.Checkbutton(section, text="Fetch Strains",
                        variable=self.fetch_strains_var).pack(anchor=tk.W)
        ttk.Checkbutton(section, text="Troubleshooting Logs",
                        variable=self.test_mode_var).pack(anchor=tk.W)
    ...
    def _build_perf_section(self, parent: ttk.Frame) -> None:
        ...
        ttk.Checkbutton(section, text="Concurrent Menus",
                        variable=self.concurrent_var).pack(anchor=tk.W)
        ttk.Checkbutton(section, text="Enable Cache",
                        variable=self.cache_var).pack(anchor=tk.W)
        ttk.Checkbutton(section, text="Optimized Processing",
                        variable=self.optimize_var).pack(anchor=tk.W)
    ...
    def _bind_events(self) -> None:
        self.slug_mode_var.trace_add(
            "write", lambda *_: self._update_slug_mode_ui())
        self.root.bind("<Return>", lambda event: self.start_button.invoke(
        ) if self.start_button.instate(["!disabled"]) else None)

    def _update_slug_mode_ui(self) -> None:
        mode = self.slug_mode_var.get()
        if mode == "single":
            self.slug_entry.configure(state=tk.NORMAL)
            self.slug_entry.focus()
        else:
            self.slug_entry.configure(state=tk.DISABLED)
    ...
    def _validate_config(self) -> RunConfig | None:
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Numbers", "Max workers/page size must be >= 1 and rate limit >= 0.")
            return None
    ...
    def _run_pipeline(self, config: RunConfig) -> None:
        ...
        try:
            ...
            self.event_queue.put(
                ("progress_text", f"Preparing run for {run_label}"))

            if config.fetch_brands:
                self.event_queue.put(
                    ("progress_text", "Fetching global brands"))
                cana.get_brands()

            if config.fetch_strains:
                self.event_queue.put(
                    ("progress_text", "Fetching global strains"))
                cana.get_strains()
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    self._finish_run(
                        success=False, summary=f"Failed: {event[1]}")
