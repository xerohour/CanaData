...
## 2026-09-22 - [Avoid .update() and comprehensions in tight loops]
**Learning:** Using dictionary comprehensions and `.update()` in the core flattening loop (`_flatten_dictionary_custom`) adds memory and processing overhead from intermediate object creation.
**Action:** Always prefer direct key assignments (e.g. `result[key] = value`) over dictionary creation + update operations in performance-sensitive loop paths.
