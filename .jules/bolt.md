...
## 2024-05-10 - Optimizing Python dictionary merging in loops
**Learning:** Using `item.copy()` followed by direct key assignment (`item['key'] = value`) is significantly faster than using `dict(item)` and `.update({})` when augmenting dictionaries inside large loops.
**Action:** Always prefer `.copy()` and direct key assignment for micro-optimizing dictionary augmentation in hot paths.
