...
## 2024-04-16 - Optimized dictionary merging
**Learning:** Iterative dictionary copying and updating `copy()` + `update()` inside loops is a bottleneck compared to Python 3.9+'s dictionary union operator `|`.
**Action:** Use dictionary union `|` inside list comprehensions (e.g. `[template | item for item in items]`) to execute dictionary merging loops at C-level speed and significantly reduce latency.
