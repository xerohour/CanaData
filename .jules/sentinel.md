...
## 2026-07-18 - XSS in URL Attributes
**Vulnerability:** URL attributes like href and src generated from external API data were only sanitized with html.escape, allowing potential execution of javascript: URIs. The HTML also lacked a Content-Security-Policy (CSP) meta tag.
**Learning:** html.escape does not protect against XSS when used in href or src attributes if the underlying URL scheme is malicious. A defense-in-depth approach is necessary.
**Prevention:** Always validate URL protocols (e.g., http://, https://) before rendering them in href or src attributes, and implement a strict Content-Security-Policy (CSP) meta tag to prevent inline script execution.
