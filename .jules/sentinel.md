...
## 2024-05-24 - [High] Prevent XSS via javascript: URIs
**Vulnerability:** XSS vulnerability in HTML link attributes via `javascript:` URIs.
**Learning:** `html.escape` does not protect against XSS when input is used in `href` attributes, as `javascript:` URIs can still be executed.
**Prevention:** Always validate that URLs start with safe protocols (`http://` or `https://`) and add a strict Content-Security-Policy (CSP) meta tag as defense-in-depth.
