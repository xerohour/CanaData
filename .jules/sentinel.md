...
## 2024-07-14 - XSS via javascript: URI in dynamically generated HTML
**Vulnerability:** Even when using html.escape(), rendering untrusted URLs in href and src attributes allowed XSS via `javascript:` URIs.
**Learning:** `html.escape` is insufficient for `href` attributes; you must also validate that URLs start with safe protocols (e.g., `http://`, `https://`) to prevent `javascript:` URI execution, and include a strict Content-Security-Policy (CSP) meta tag as defense-in-depth.
**Prevention:** Use a safe URL validation function enforcing HTTP/HTTPS schemes for external links and include a strong CSP meta tag.
