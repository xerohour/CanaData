...
## 2024-07-17 - XSS via javascript: URIs in HTML Generator
**Vulnerability:** Even though user input was escaped using `html.escape()`, `javascript:` URIs could still be injected into `href` and `src` attributes, allowing Cross-Site Scripting (XSS) if clicked.
**Learning:** `html.escape()` is not sufficient for URLs, as it does not prevent the use of malicious URI schemes. A Content-Security-Policy (CSP) and strict URL parsing are required.
**Prevention:** Always validate URL schemes (e.g., using `urllib.parse`) to ensure they are safe (`http`, `https`, `mailto`, `tel`) before rendering them in attributes, and implement a strict Content-Security-Policy header.
