...
class CanaData:
    ...
    def _getMenusConcurrent(self):
        ...
        for slug, result in results.items():
            if result:
                ...
                if result['empty_menu']:
                    self.emptyMenus[listing_id] = result['empty_menu']
                for slug_strain, strain in result['extracted_strains'].items():
                    if slug_strain not in self.extractedStrains:
                        self.extractedStrains[slug_strain] = strain
                self.menuItemsFound += result['menu_items_count']
                self.totalLocations.append(result['listing_copy'])
    ...
    def process_menu_json(self, menu_json: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        Returns the processed data for aggregation.
        """
