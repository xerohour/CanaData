...
class CanaData:
    ...
    def flatten_dictionary(self, d: Dict[str, Any]) -> Dict[str, str]:
        ...
        # ⚡ Bolt Optimization: pre-cache string join for O(1) attribute lookup in tight loop
        join_keys = '.'.join
        ...
        while stack:
            for k, v in stack[-1]:
                ...
                if isinstance(v, list):
                    ...
                    # ⚡ Bolt Optimization: implicit boolean evaluation instead of len(v) > 0
                    if v:
                        for item in v:
                            if item:
                                if isinstance(item, dict):
                                    # ⚡ Bolt Optimization: implicit boolean evaluation instead of len(item.keys()) < 1
                                    if not item:
                    ...
                    # ⚡ Bolt Optimization: implicit boolean evaluation instead of len(v.keys()) < 1
                    if not v:
                        result[join_keys(keys)] = 'None'
                        keys.pop()
                    else:
