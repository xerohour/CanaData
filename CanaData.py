...
class CanaData:
    ...
    def _getMenusConcurrent(self):
        ...
        # Update instance variables with results sequentially
        for location_slug, result in processor.results.items():
            if result:
                self._aggregate_menu_result(result)
    ...
    def _aggregate_menu_result(self, result: Dict[str, Any]) -> None:
        """Aggregate results from stateless worker threads on the main thread."""
        listing_id = result['listing_id']
        self.allMenuItems[listing_id] = result['menu_items']

        if result.get('empty_menu'):
            self.emptyMenus[listing_id] = result['empty_menu']
    ...
    def process_menu_json(self, menu_json: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'empty_menu': listing_copy if is_empty_menu else None,
    ...
    def process_menu_items_json(self, menu_json: Dict[str, Any], location: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            'listing_id': listing_id,
            'menu_items': local_menu_items,
            'empty_menu': listing_copy if menu_items_count == 0 else None,
            'extracted_strains': local_extracted_strains,
            'menu_items_count': menu_items_count,
            'listing_copy': listing_copy
        }
