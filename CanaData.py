...
class CanaData:
    ...
    def _getMenusSequential(self):
        ...
        for i, location in enumerate(self.locations):
            ...
            if result is not None and result is not False:
                self._aggregate_menu_data(result)
    ...
    def _aggregate_menu_data(self, data: Dict[str, Any]) -> None:
        """Aggregate data returned by worker threads safely on the main thread."""
        listing_id = data.get('listing_id')
        if listing_id:
            self.allMenuItems[listing_id] = data['local_menu_items']
            if data.get('is_empty_menu'):
                self.emptyMenus[listing_id] = data['listing_copy']

        for slug, strain in data.get('local_extracted_strains', {}).items():
            if slug not in self.extractedStrains:
                self.extractedStrains[slug] = strain

        self.menuItemsFound += data.get('menu_items_count', 0)

        if data.get('listing_copy'):
