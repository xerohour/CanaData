...
class CanaData:
    ...
    def _aggregate_results(self) -> None:
        """Flushes the data queue and aggregates results into thread-safe state variables."""
        while not self._data_queue.empty():
            ...
            local_menu_items = data["local_menu_items"]
            is_empty_menu = data.get("is_empty_menu", False)
            listing_copy = data["listing_copy"]
            local_extracted_strains = data["local_extracted_strains"]
            menu_items_count = data["menu_items_count"]

            if listing_id not in self.allMenuItems:
                self.allMenuItems[listing_id] = []
            self.allMenuItems[listing_id].extend(local_menu_items)

            if is_empty_menu or menu_items_count == 0:
