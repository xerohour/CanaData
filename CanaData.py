...
class CanaData:
    ...
    def _original_organize_into_clean_list(self):
        ...
        for item in flatDictList:
            all_keys_set.update(item)

        all_keys = sorted(list(all_keys_set))

        template_dict = dict.fromkeys(all_keys, 'None')

        # Using Python 3.9+ dictionary union operator (|) to merge the base keys with actual values
        # This performs significantly faster than iteratively calling dict.copy() and dict.update()
