...
class CanaData:
    ...
    def flatten_dictionary(self, d: Dict[str, Any]) -> Dict[str, str]:
        ...
        # ⚡ Bolt Optimization: Cache list/string builtins to reduce attribute lookup overhead in tight loop
        ...
        while stack:
            for k, v in stack[-1]:
                keys_append(k)
                # ⚡ Bolt Optimization: Replace isinstance(v, dict) with exact type checks to avoid MRO overhead
                if type(v) is dict:
                    # Handle nested dictionaries
                    # ⚡ Bolt Optimization: Replace len(v.keys()) < 1 with implicit boolean evaluation 'if not v:'
                    if not v:
                    ...
                    # ⚡ Bolt Optimization: Replace len(v) > 0 with implicit boolean evaluation 'if v:'
                    if v:
                        for item in v:
                            if item:
