...
class CanaData:
    ...
    def __init__(
        ...
        self.maxLocations: int | None = None
    ...
    def do_request(self, url, use_cache: bool = True):
        ...
        try:
            ...
            logger.error(f"Network error occurred: {e!s}")
    ...
    def getLocations(self, lat: float | None = None, long: float | None = None) -> None:
        ...
        while True:
            ...
            url = f"{self.baseUrl}?offset={self.locationsFound!s}{self.pageSize}"
    ...
    def getBrands(self) -> None:
        ...
        offset: int = 0
        while True:
            url = f"{self.brandsBaseUrl}?offset={offset!s}{self.pageSize}"
    ...
    def getStrains(self) -> None:
        ...
        offset: int = 0
        while True:
            url = f"{self.strainsBaseUrl}?offset={offset!s}{self.pageSize}"
    ...
    def _original_organize_into_clean_list(self):
        ...
        # Convert all_keys to a tuple for faster iteration
        _all_keys = tuple(all_keys)

        # Loop through the flatDictList to update any missing keys
        for item in flatDictList:
            # Copy the item directly and manually pad missing keys (faster than dict.copy + update)
            flat_ordered_dict = item.copy()
            for k in _all_keys:
                if k not in flat_ordered_dict:
                    flat_ordered_dict[k] = "None"
    ...
    def flatten_dictionary(self, d: dict[str, Any]) -> dict[str, str]:
        ...
        # Local method and type caching for performance
        _dict = dict
        _list = list
        _join = ".".join
        _str = str
    ...
    def csv_maker(
        ...
        with open(
            f"{home_dir}/{sanitized_filename}.csv", "w", newline="", encoding="utf-8"
        ) as outfile:
            ...
            print(
                f"Successfully exported ({len(data)!s} items) to CSV -> {sanitized_filename}.csv"
            )
    ...
    def dataToCSV(self) -> None:
        ...
        print(
            f"\n\nResults for -> {self.searchSlug}:\n- {self.locationsFound!s} Locations\n- {len(self.allMenuItems.keys())!s} Menus\n- {len(self.emptyMenus.keys())!s} Empty Menus\n- {self.menuItemsFound!s} Menu Items"
        )
