...
class CanaData:
    ...
    def _aggregate_menu_result(self, result: Dict[str, Any]) -> None:
        ...
        if result['is_empty']:
            self.emptyMenus[listing_id] = result['listing']

        for slug, strain in result['strains'].items():
            if slug not in self.extractedStrains:
                self.extractedStrains[slug] = strain

        self.menuItemsFound += result['menu_items_count']
        self.totalLocations.append(result['listing'])
    ...
    def process_menu_json(self, menu_json: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'is_empty': is_empty_menu,
            'strains': local_extracted_strains,
