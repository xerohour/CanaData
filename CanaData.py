...
class CanaData:
    ...
    def _aggregate_results(self):
        """Flush the result_queue to update instance state synchronously."""
        while not self.result_queue.empty():
            ...
            local_menu_items = result.get('local_menu_items', [])
            listing_copy = result.get('listing_copy')
            is_empty_menu = result.get('is_empty_menu', False)
            menu_items_count = result.get('menu_items_count', 0)
            local_extracted_strains = result.get('local_extracted_strains', {})

            self.allMenuItems[listing_id] = local_menu_items
            if is_empty_menu and listing_copy:
