...
class CanaData:
    ...
    def _getMenusSequential(self):
        ...
        for i, location in enumerate(self.locations):
            ...
            self._aggregate_menu_result(result)
    ...
    def _getMenusConcurrent(self):
        ...
        # Synchronously aggregate all returned results in the main thread
    ...
    def _aggregate_menu_result(self, result: Optional[Dict[str, Any]]) -> None:
        """Aggregates a single processed menu result into global state without locks.

        Optimization: By returning data functionally from worker threads and aggregating
        it synchronously in the main thread, we eliminate the need for `_menu_data_lock`.
        This prevents lock contention (the "noisy neighbor" problem) under high
        concurrency, allowing linear horizontal scaling.
        """
        if not result:
            return
    ...
    def process_menu_json(self, menu_json: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        listing = menu_json.get('listing') or {}
