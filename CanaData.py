...
class CanaData:
    ...
    def _original_organize_into_clean_list(self):
        ...
        # Pre-initialize a template dictionary with all keys set to 'None' for faster instantiation
        template_dict = dict.fromkeys(all_keys, 'None')

        # Loop through the flatDictList to update any missing keys
        for item in flatDictList:
            # Copy template to avoid dictionary comprehension overhead
            flat_ordered_dict = template_dict.copy()
            # Update with actual values (values are already strings from flatten_dictionary)
