...
class CanaData:
    ...
    def _getMenusConcurrent(self):
        ...
        # Update instance variables with results synchronously to avoid lock contention
        self._aggregate_menu_results(results)
    ...
    def _fetch_and_process_menu(self, location: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Fetch and process menu for a single location. Returns a dict of processed data instead of modifying state."""
        location_slug = location["slug"]
        ...
        try:
            ...
            if discovery_items is not None:
                return self._parse_menu_items_json(discovery_items, location)
            ...
            if resp.status_code == 200:
                return self._parse_menu_json(resp.json())
    ...
    def _aggregate_menu_results(self, results: Dict[str, Any]) -> None:
        """Synchronously aggregates results from concurrent workers into shared state."""
        for location_slug, result in results.items():
            ...
            # Avoid lock contention by doing this synchronously in the main thread
            self.allMenuItems[listing_id] = result.get('local_menu_items', [])

            if result.get('is_empty_menu'):
                self.emptyMenus[listing_id] = result.get('listing_copy')

            for slug, strain in result.get('local_extracted_strains', {}).items():
                if slug not in self.extractedStrains:
                    self.extractedStrains[slug] = strain

            self.menuItemsFound += result.get('menu_items_count', 0)
            self.totalLocations.append(result.get('listing_copy'))
    ...
    def _parse_menu_json(self, menu_json: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Process the JSON response for a single location's menu. Returns dict of parsed data.
        """
    ...
    def _parse_menu_items_json(self, menu_json: Dict[str, Any], location: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Process discovery/v1/listings/{type}/{slug}/menu_items payload. Returns dict of parsed data."""
