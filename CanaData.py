...
class CanaData:
    ...
    def __init__(
        ...
        if cache_enabled:
            ...
            self.cache_manager = CacheManager(
                ...
                enable_disk_cache=os.getenv(
                    'ENABLE_DISK_CACHE', 'true').lower() == 'true'
            )
    ...
    def do_request(self, url, use_cache: bool = True):
        ...
        if self.cache_enabled and self.api_client and use_cache:
            try:
                ...
                logger.warning(
                    f"Cached request failed, trying without cache: {e}")
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                try:
                    error_detail = req.json().get('errors', [{}])[
                        0].get('detail', req.text)
    ...
    def _do_curl_request(self, url: str):
        ...
        try:
            completed = subprocess.run(
                curl_cmd, capture_output=True, timeout=45)
            ...
            if status_code == 422:
                try:
                    error_detail = json.loads(body).get('errors', [{}])[
                        0].get('detail', body)
                    logger.error(
                        f"Validation Error (422 via curl): {error_detail}")
                except Exception:
    ...
    def getLocations(self, lat: Optional[float] = None, long: Optional[float] = None) -> None:
        ...
        if not self.searchSlug:
            logger.error(
                "No search slug provided! Use -go <slug> or specify a slug.")
            return
        ...
        while True:
            ...
            if locations:
                ...
                if self.maxLocations is None:
                    ...
                    logger.info(
                        f"Set the max locations # to {self.maxLocations}")
    ...
    def getMenus(self) -> None:
        ...
        use_concurrent = os.getenv(
            'USE_CONCURRENT_PROCESSING', 'false').lower() == 'true'
    ...
    def _getMenusSequential(self):
        ...
        for i, location in enumerate(self.locations):
            ...
            self._merge_menu_results(result)
    ...
    def _getMenusConcurrent(self):
        ...
        logger.info(
            f"Processing {len(self.locations)} locations concurrently...")
        ...
        results = processor.process_locations(
            self.locations, process_location_menu)

        # Merge results sequentially in main thread
    ...
    def _fetch_and_process_menu(self, location: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            resp = requests.get(
                legacy_url, headers=self.default_headers, timeout=30)
    ...
    def _merge_menu_results(self, result: Optional[Dict[str, Any]]) -> None:
        if not result:
            return

        for listing_id, items in result.get('allMenuItems', {}).items():
            if listing_id not in self.allMenuItems:
                self.allMenuItems[listing_id] = []
            self.allMenuItems[listing_id].extend(items)

        for listing_id, empty_menu in result.get('emptyMenus', {}).items():
            if listing_id not in self.emptyMenus:
                self.emptyMenus[listing_id] = empty_menu

        for slug, strain in result.get('extractedStrains', {}).items():
            if slug not in self.extractedStrains:
                self.extractedStrains[slug] = strain

        self.menuItemsFound += result.get('menuItemsFound', 0)
        self.totalLocations.extend(result.get('totalLocations', []))
        ...
        while True:
            ...
            if data and data != 'break':
                ...
                if not strains_list:
                    ...
                    strains_list = data.get('data', {}).get(
                        'taxonomy', {}).get('strains', [])
                    if not strains_list:
                        break
    ...
    def process_menu_json(self, menu_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if listing_id is None:
            logger.warning(
                f"Skipping menu without listing id for {listing_slug}")
            return {}

        listing_type = 'deliveries' if listing.get(
            '_type') == 'delivery' else 'dispensaries'
        ...
        result_dict = {
            'allMenuItems': {listing_id: local_menu_items},
            'emptyMenus': {listing_id: listing_copy} if is_empty_menu else {},
            'extractedStrains': local_extracted_strains,
            'menuItemsFound': menu_items_count,
            'totalLocations': [listing_copy]
        }
    ...
    def process_menu_items_json(self, menu_json: Dict[str, Any], location: Dict[str, Any]) -> Dict[str, Any]:
        ...
        listing_type = self._to_listing_path_type(
            location.get('type', 'dispensary'))
        listing_url = f'/{listing_type}/{listing_slug}'
        ...
        result_dict = {
            'allMenuItems': {listing_id: local_menu_items},
            'emptyMenus': {listing_id: listing_copy} if menu_items_count == 0 else {},
            'extractedStrains': local_extracted_strains,
            'menuItemsFound': menu_items_count,
            'totalLocations': [listing_copy]
        }
    ...
    def getLeaflyData(self):
        ...
        if leafly_items:
            ...
            logger.info(
                f"Successfully integrated {self.menuItemsFound} Leafly items.")
        else:
    ...
    def getCannMenusData(self):
        ...
        if not retailers:
            logger.warning(
                f"No retailers found on CannMenus for: {self.searchSlug}")
            return
    ...
    def flatten_dictionary(self, d: Dict[str, Any]) -> Dict[str, str]:
        ...
        result = {}
        # Stack contains iterators of dictionary items
        stack = [iter(d.items())]
        # Tracks the current path in the dictionary (e.g., ['price', 'amount'])
        keys = []
        while stack:
            for k, v in stack[-1]:
                keys.append(k)
    ...
    def csv_maker(self, filename: str, data: List[Dict[str, Any]], preorganized: bool = False) -> None:
        ...
        with open(f'{home_dir}/{sanitized_filename}.csv', 'w', newline='', encoding='utf-8') as outfile:
            ...
            print(
                f'Successfully exported ({str(len(data))} items) to CSV -> {sanitized_filename}.csv')
    ...
    def dataToCSV(self) -> None:
        ...
        try:
            self.csv_maker(f'{self.searchSlug}_results',
        ...
        try:
            self.csv_maker(f'{self.searchSlug}_total_listings',
                           self.totalLocations)
        except Exception as e:
            print(f'Error: {str(e)}')
            print(
