...
class CanaData:
    ...
    def flatten_dictionary(self, d: Dict[str, Any]) -> Dict[str, str]:
        ...
        join = '.'.join           # Performance: Pre-cache built-in string method
        while stack:
            for k, v in stack[-1]:
                keys.append(k)
                v_type = type(v)
                if v_type is list:
                    ...
                    if v:  # Performance: Implicit boolean eval instead of len(v) > 0
                        for item in v:
                            if item:
                                item_type = type(item)
                                if item_type is dict:
                                    if not item:  # Performance: Implicit boolean eval instead of len(item.keys()) < 1
                    ...
                    if not v:  # Performance: Implicit boolean eval instead of len(v.keys()) < 1
                        result[join(keys)] = 'None'
                        keys.pop()
                    else:
                        # Push the nested dict onto the stack
