...
class CacheManager:
    ...
    def _prune_memory_cache(self) -> None:
        ...
        while len(self.memory_cache) > self.memory_cache_size:
            oldest_key = min(
                self.memory_cache, key=lambda key: self.memory_cache[key]["timestamp"]
            )
    ...
    def _get_from_disk(self, cache_key: str) -> Optional[Any]:
        ...
        if not cache_file.exists():
            # Check legacy cache for backwards compatibility, then upgrade it
            legacy_file = self.cache_dir / f"{cache_key}.cache"
    ...
    def invalidate(self, pattern: Optional[str] = None) -> None:
        ...
        if pattern:
            ...
            for cache_file in list(self.cache_dir.glob("*.cache")) + list(
        ...
        return {
            **self.stats,
            ...
            "disk_cache_files": len(
                list(self.cache_dir.glob("*.cache"))
