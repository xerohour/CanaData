...
class CacheManager:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        disk_cache_count = len(list(self.cache_dir.glob("*.json_cache"))) + len(list(self.cache_dir.glob("*.cache")))

        return {
            **self.stats,
            'hit_rate_percent': hit_rate_float,
            'memory_cache_size': len(self.memory_cache),
            'disk_cache_files': disk_cache_count if self.enable_disk_cache else 0
        }
