...
def test_get_brands_success(cana):
    mock_url = (
        "https://api-g.weedmaps.com/discovery/v1/brands?offset=0&page_size=100&size=100"
    )
    mock_response = {
        "meta": {"total_brands": 1},
...
def test_get_strains_success(cana):
    ...
    mock_response = {
        ...
        "data": {"strains": [{"id": 101, "name": "Blue Dream", "slug": "blue-dream"}]},
    }
    responses.add(responses.GET, mock_url, json=mock_response, status=200)
...
def test_cannmenus_get_retailers(monkeypatch):
    ...
    mock_response = {"data": [{"id": "shop-1", "name": "NYC Dispensary"}]}
    responses.add(responses.GET, mock_url, json=mock_response, status=200)
...
def test_cannmenus_get_menu(monkeypatch):
    ...
    mock_response = {"data": [{"name": "Item 1", "price": 50}]}
    responses.add(responses.GET, mock_url, json=mock_response, status=200)
...
def test_extract_strains_from_menu(cana):
    ...
    mock_item = {
        "name": "OG Kush",
        "strain_data": {"slug": "og-kush", "name": "OG Kush", "genetics": "hybrid"},
        "id": 101,
    ...
    mock_menu_json = {
        ...
        "categories": [{"title": "Flower", "items": [mock_item]}],
    }
