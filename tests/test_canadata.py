...
def test_process_menu_json_thread_safe_counts_and_collections():
    ...
    payloads = [
        _build_menu_payload(
            listing_id=f'loc-{i}', slug=f'slug-{i}', item_count=items_per_payload)
        for i in range(total_payloads)
    ]

    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(cana.process_menu_json, payload)
                   for payload in payloads]
        for future in concurrent.futures.as_completed(futures):
            result = future.result()
            cana._merge_menu_results(result)
...
def test_process_menu_json_thread_safe_deduplicates_extracted_strains():
    ...
    payloads = [
        _build_menu_payload(
            listing_id=f'loc-{i}', slug=f'slug-{i}', item_count=1, strain_slug='same-strain')
        for i in range(total_payloads)
    ]
