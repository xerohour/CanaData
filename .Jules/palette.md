...
## 2026-08-01 - Dashboard Skip Link and Smooth Scroll
**Learning:** The dashboard `index.html` generated by `CanaParse.py` lacked a skip-to-content link, and its anchor navigation was jarring due to the large data tables.
**Action:** Added `.skip-link` and `scroll-behavior: smooth` to the global CSS in `CanaParse.py` so that users relying on keyboard navigation can effectively skip the navbar and the anchor links feel less disorienting.
