...
## 2026-08-13 - Smooth Scrolling for Anchor Links
**Learning:** Adding smooth scrolling improves the UX of anchor links (like the skip link and navbar links) but must be wrapped in a `@media (prefers-reduced-motion: no-preference)` query to respect users with vestibular disorders and meet accessibility guidelines.
**Action:** Always wrap `scroll-behavior: smooth;` in a `@media (prefers-reduced-motion: no-preference)` query when adding smooth scrolling to UI elements.
