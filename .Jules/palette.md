...
## 2024-08-21 - Accessible Navigation and Smooth Scrolling
**Learning:** Adding smooth scrolling improves UX for anchor link navigation, but can cause accessibility issues for users with vestibular disorders if not wrapped in a prefers-reduced-motion media query. Furthermore, complex interfaces with navigation bars require a skip-to-content link to allow keyboard users to bypass navigation. The target container for this link must have tabindex="-1" to properly receive focus programmatically without disrupting the natural document tab order.
**Action:** Always wrap scroll-behavior: smooth in @media (prefers-reduced-motion: no-preference) and implement a skip-to-content link that targets a container with tabindex="-1" and an id.
