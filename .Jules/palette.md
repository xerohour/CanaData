...
## 2026-08-15 - Smooth Scrolling
**Learning:** Adding `scroll-behavior: smooth;` improves UX for anchor links but requires wrapping in a `@media (prefers-reduced-motion: no-preference)` query to respect accessibility settings for users with vestibular disorders.
**Action:** Always wrap smooth scrolling styles in a prefers-reduced-motion media query to ensure accessibility.
